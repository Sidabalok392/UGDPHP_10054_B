<?php
    ...
    $id=$_GET['id'];
    $query=mysqli_query($con,"SELECT * FROM prodi WHERE id = '$id'") or die(mysqli_error($con));
...
<h4 >EDIT PRODI</h4>
<hr>
<form action="../process/editProdiProcess.php?id= <?php echo $id?>" method="post">
<div class="mb-3">
    <label for="exampleInputEmail1" class="form-label">Nama Fakultas</label>
    <input class="form-control" id="fakultas" name="fakultas" aria-describedby="emailHelp" value="<?php echo $data['fakultas']?>">
</div>
<div class="mb-3">
    <label for="exampleInputEmail1" class="form-label">Nama Prodi</label>
    <input class="form-control" id="prodi" name="prodi" aria-describedby="emailHelp" value="<?php echo $data['prodi']?>">
</div>
<div class="d-grid gap-2">
    <button type="submit" class="btn btn-primary" name="register">Edit Prodi</button>
</div>
</form>
</div>
</aside>
